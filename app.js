...
function validateSession (username, sessionToken, resp, successCallback) {
  ...
  if (username in database) {
    const userData = database[username];
...
function getCircuitFile (username, resp, successCallback) {
  ...
  const circuitPath = `secrets/circuits/${username}.json`;
...
function postCreateAccount (req, resp) {
  ...
  resp.status(200).send({ sessionToken });
...
function postLogin (req, resp) {
  ...
  if (!(username in database)) {
    return resp.status(404).send(responseJSON(USER_DOESNT_EXIST_MESSAGE));
  }
...
function getUsers (req, resp) {
  ...
  for (const username in database) {
    const entry = database[username];
...
function getUser (req, resp) {
  ...
  return getCircuitFile(username, resp, (database, circuitFile, path) => {
    ...
    for (const circuitName in circuitFile) {
      const entry = circuitFile[circuitName];
...
function getCircuits (req, resp) {
  ...
  return getCircuitFile(username, resp, (database, circuitFile, path) => {
    ...
    return resp.status(200).send({ circuitNames });
  });
}

function getCircuit (req, resp) {
  const username = req.query.username;
  const circuitName = req.query.circuitName;
...
function postSaveCircuit (req, resp) {
  ...
  const circuitData = req.body.circuitData;
...
module.exports = { app, isValidPassword, isValidText, responseJSON };
